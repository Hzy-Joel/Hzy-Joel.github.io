...
以上代大部分是在注册对应的解码器，直接看最后一段，构建了一个GlideContext对象保存了各类的属性。
**以上就是Glide的初始化过程中各类资源的创建过程，回到初始化的 第 2 点，Glide的get方法触发了Glide的初始化，并在上述代码中初始化了RequestManagerRetriever对象，Glide的with方法实际调用的是RequestManagerRetriever的get方法**
